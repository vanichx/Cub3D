...
void update_texts_pixels(t_cube *cube, t_ray *ray, int x)
{
    ...
    cube->wall_text.text_point[X] = (int)(ray->wall_x * cube->wall_text.tex_size);
    if (((ray->side == 0) && (ray->ray_dir.dir[X] < 0))
        || ((ray->side == 1) && (ray->ray_dir.dir[Y] > 0)))
        cube->wall_text.text_point[X] = cube->wall_text.tex_size - cube->wall_text.text_point[X] - 1;
    cube->wall_text.tex_step = 1.0 * cube->wall_text.tex_size / ray->line_height;
    cube->wall_text.tex_pos = (ray->draw[START] - cube->screen.height / 2 + ray->line_height / 2) * cube->wall_text.tex_step;
    y = ray->draw[START];
    while (y < ray->draw[END])
    {
        cube->wall_text.text_point[Y] = (int)cube->wall_text.tex_pos & (cube->wall_text.tex_size - 1);
        cube->wall_text.tex_pos += cube->wall_text.tex_step;
        color = cube->wall_text.textures[texture_index][cube->wall_text.tex_size * cube->wall_text.text_point[Y] + cube->wall_text.text_point[X]];
